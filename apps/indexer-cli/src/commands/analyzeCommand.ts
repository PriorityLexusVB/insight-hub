...
function fallbackClassifyUnknown(params: {
  title: string;
  bodyText: string;
...
export async function runAnalyzeCommand(
  ...
  for (const file of threadFiles) {
    ...
    const { is_work: isWorkBase, work_type: workTypeBase } = classifyWork({
      meta: metaForClassify,
      title,
      bodyText: bodyNoCode,
    });

    let is_work = isWorkBase;
    let work_type = workTypeBase;

    // Deterministic fallback: if still unknown, classify from title/body keywords.
    if (domainEffective === "unknown") {
      const fb = fallbackClassifyUnknown({ title, bodyText: bodyNoCode });
      if (fb) {
        domainEffective = fb.domain;
        work_type = fb.work_type;
        is_work =
          domainEffective !== "personal" &&
          work_type !== "personal" &&
